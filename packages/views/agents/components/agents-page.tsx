...
export function AgentsPage() {
  ...
  const handleUpdate = async (id: string, data: Record<string, unknown>) => {
    try {
      ...
      qc.invalidateQueries({ queryKey: workspaceKeys.agents(wsId) });
      toast.success("数字员工信息已更新");
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "更新数字员工失败");
      throw e;
    }
  };
  ...
  const handleArchive = async (id: string) => {
    try {
      ...
      qc.invalidateQueries({ queryKey: workspaceKeys.agents(wsId) });
      toast.success("数字员工已归档");
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "归档数字员工失败");
    }
  };
  ...
  const handleRestore = async (id: string) => {
    try {
      ...
      qc.invalidateQueries({ queryKey: workspaceKeys.agents(wsId) });
      toast.success("数字员工已恢复");
    } catch (e) {
      toast.error(e instanceof Error ? e.message : "恢复数字员工失败");
    }
  };
    ...
    >
      <ResizablePanel id="list" defaultSize={280} minSize={240} maxSize={400} groupResizeBehavior="preserve-pixel-size">
        ...
        <div className="overflow-y-auto h-full border-r">
          <div className="flex h-12 items-center justify-between border-b px-4">
            <h1 className="text-sm font-semibold">数字员工</h1>
            <div className="flex items-center gap-1">
              {archivedCount > 0 && (
                <Button
                  ...
                  title={showArchived ? "查看在线数字员工" : "查看已归档数字员工"}
                >
          ...
          {filteredAgents.length === 0 ? (
            <div className="flex flex-col items-center justify-center px-4 py-12">
              ...
              <p className="mt-3 text-sm text-muted-foreground">
                {showArchived ? "还没有已归档数字员工" : archivedCount > 0 ? "还没有在线数字员工" : "还没有数字员工"}
              </p>
              {!showArchived && (
                <Button
                  onClick={() => setShowCreate(true)}
      ...
      <ResizablePanel id="detail" minSize="50%">
        ...
        {selected ? (
          ...
          <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
            ...
            <p className="mt-3 text-sm">选择一个数字员工查看详情</p>
            <Button
              onClick={() => setShowCreate(true)}
              size="xs"
              className="mt-3"
