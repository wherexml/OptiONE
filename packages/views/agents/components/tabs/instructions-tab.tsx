...
export function InstructionsTab({
  ...
  return (
    <div className="space-y-4">
      <div>
        <h3 className="text-sm font-semibold">数字员工说明</h3>
        <p className="text-xs text-muted-foreground mt-0.5">
          这里定义这个数字员工的角色、判断方式和工作规则。系统在每次分配任务时都会把这段说明带给它。
        </p>
      </div>
      ...
      <textarea
        ...
        placeholder={`请用业务语言说明这个数字员工要负责什么、怎么判断、输出什么。\n\n示例：\n你是供应链异常处理 Agent，负责识别库存、需求和供应异常，并把告警整理成任务。\n\n## 输出要求\n- 先写结论，再写依据\n- 缺失数据必须单独列出\n- 必须说明建议 / 审批 / 自动三种模式里该选哪一种\n\n## 边界\n- 没有数字证据时不能下结论\n- 不直接触发高风险外部写操作`}
      ...
      <div className="flex items-center justify-between">
        <span className="text-xs text-muted-foreground">
          {value.length > 0 ? `已填写 ${value.length} 个字符` : "还没有填写数字员工说明"}
        </span>
        <Button
          size="xs"
          onClick={handleSave}
